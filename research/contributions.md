...
|---|---|---|---|---|
| **S0** | **The retrieval substrate outperforms a deployed memory architecture, and the gap widens with scale** — *headline claim* | **+13.1pp BEAM-500K, +42.7pp LOCOMO** over the cognitive-core KB baseline; scale curve parity@100K → +14.7@500K → +23.2@1M → **+28.3 ±7.2 @10M** (29.9 vs 58.3, paired t=7.91, p<0.01); mechanism is KB log-dump truncation (~69% dropped at 1M) vs size-invariant focused retrieval; three-arm decomposition isolates it from the graph (+1.9/+1.2, inside noise). **At 10M the flat arm is 4x more stable across conversations than the baseline** (sd 1.6 vs 6.3), and delta rank-correlates −1 with baseline score — the substrate wins most where the baseline struggles most | mean-3 @BEAM 500K/1M; 300q @LOCOMO; **10M is n=4 conversations paired, samples=1 per cell** | **NOT CHECKED.** "Hybrid beats vector-only" is well established (see novelty constraints). The claim here is a *system result against a deployed baseline*, not a technique claim — but no search has been run. **Highest-priority gap now that this leads** |
...
| S4 | **Compression buys cost, not quality** *(rewritten 2026-08-02 — the trade framing is refuted, see R8)* | At coverage-matched k=32, extraction's synthesis edge vanishes exactly (85.3 vs 85.3); recall goes to verbatim by 13.3, overall by 6.5. Extraction's actual purchase is **cost**: near-parity synthesis at ~1/7 the context tokens | n=1 per arm, 3 arms | close to a reproduction; see novelty constraints below. Prediction C1-P recorded in the ledger, untested |
...
| **R8** | **"Compression trades recall for synthesis, at ~−12.2 for +5.9"** (**ours, held for one day**) | **The coverage-matched control.** At k=32 the synthesis edge is exactly zero (85.3 vs 85.3) and verbatim wins overall by 6.5. The +5.9 was a **retrieval-budget artifact**. Note the failure mode: the budget objection was raised, then dismissed by measuring *tokens* (verbatim already had 3.7x more) when the binding axis was *coverage*. Argument did not catch it; only the control did |
